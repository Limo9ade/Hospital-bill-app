...
<div class="max-w-5xl mx-auto mt-12 p-6 bg-white rounded shadow">
    ...
    <!-- Page Heading -->
    <h1 class="text-2xl font-semibold mb-6 text-center">👨‍⚕️ All Doctors</h1>
    ...
    <!-- Search Form -->
    <form method="GET" action="{{ route('admin.doctors.index') }}" class="mb-6 flex flex-col sm:flex-row sm:items-center sm:space-x-2 space-y-2 sm:space-y-0">
        <input
            type="text"
            name="search"
        ...
        @if(request('search'))
            <a href="{{ route('admin.doctors.index') }}" class="text-gray-600 underline ml-0 sm:ml-4">Clear Search</a>
        @endif
    </form>

    <!-- Import Form -->
    <form action="{{ route('admin.doctors.import') }}" method="POST" enctype="multipart/form-data" class="mb-6">
        @csrf
        <label for="file" class="block mb-2 font-semibold">Import Doctors (CSV, XLSX, XLS):</label>
        ...
        <table class="min-w-full bg-white border border-gray-200 rounded shadow-md">
            <thead class="bg-gray-200 text-gray-700">
                <tr>
                    ...
                    <th class="py-3 px-6 text-left border-b border-gray-300">Visiting Time</th>
                    <th class="py-3 px-6 text-left border-b border-gray-300">Speciality</th>
            ...
            <tbody>
                @forelse ($doctors as $doctor)
                    <tr class="hover:bg-gray-50 cursor-pointer">
                        ...
                        <td class="py-3 px-6 border-b border-gray-300">{{ $doctor->name }}</td>
                        <td class="py-3 px-6 border-b border-gray-300">{{ $doctor->visiting_time }}</td>
                        <td class="py-3 px-6 border-b border-gray-300">{{ $doctor->speciality }}</td>
                        <td class="py-3 px-6 border-b border-gray-300 space-x-3">
                            <a href="{{ route('admin.doctors.edit', $doctor->id) }}" class="text-blue-600 hover:underline">Edit</a>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-4 text-center text-gray-500">No doctors found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
